...
class CameraHandshakeManager(private val context: Context) {
    ...
    companion object {
        private val COLOR_PALETTE_HUES = floatArrayOf(
            200f, 140f, 280f, 30f, 170f, 60f, 320f, 0f
        )
    }

    fun generateDisplayColor(): Int {
        val index = ((peerId.hashCode() and 0x7FFFFFFF) % COLOR_PALETTE_HUES.size)
        return Color.HSVToColor(floatArrayOf(COLOR_PALETTE_HUES[index], 0.65f, 0.95f))
    }
    ...
    fun startDiscovery() {
        ...
        discoveryListener = object : NsdManager.DiscoveryListener {
            ...
            override fun onServiceFound(serviceInfo: NsdServiceInfo?) {
                serviceInfo?.let { if (it.serviceName != "flash-$peerId") resolveService(it) }
            }
